...
# Stepwise regression procedure to find the most important variables and remove highly collinearity between the predictors.
m1 <- step(full,trace=TRUE, direction = 'both')

# Logistic model with remaining variables.
logit1 <- glm(m1 ,data = data, family = "binomial")

# Calculate the variance inflation factor for each variable to quantify the multicollinearity of our model.
vif(logit1)
sort(vif(logit1), decreasing = TRUE)

# Remove variable emp.var.rate from the model.
...
# Remove variable pdays from the model.
...
# Remove variable euribor3m from the model.
...
# After that procedure, we will pick the preferred model and test it to the validation data. In this project we will use the following machine learning algorithms :
  • Naïve Bayes Classifier
  • Support Vector Machine when using the radial basis function kernel (RBF)
